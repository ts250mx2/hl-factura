...
| **Descargar CSF con e.firma (beta)** | Replica el flujo real del portal: lanzador de "Genera tu Constancia" (`wwwmat.sat.gob.mx`, `tipoLogeo=f`) → login `login.siat.sat.gob.mx` contrato `fiel_Aviso` → firma del reto `tokenuuid\|rfc\|noSerie` con la FIEL (RSA-SHA1, igual que el JS del SAT) → regreso SAML y descarga del PDF. Si lo logra, guarda el PDF y actualiza régimen y obligaciones. Automatización de portal (el SAT no tiene API oficial), por eso sigue como **beta** con diagnóstico paso a paso. |
